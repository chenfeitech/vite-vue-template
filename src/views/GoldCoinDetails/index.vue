...
const getDataSource = async (key: TabKeyType) => {
  try {
    const resp = await api.test.goldCoins({
      page: state[key].page,
      pageSize: 20,
    });
...
watchEffect(() => {
  switch (state.active) {
    case 0:
      ...
      break;
    case 1:
      state["contributionTab"].data === null && getDataSource("contributionTab");
      break;
    case 2:
...
<template>
  <div :class="['page', pageCls]">
    ...
    <!-- 导航栏 -->
    <app-header v-if="env !== 'weixin'" title="金币明细" theme="light" backgroundColor="transparent" :showBack="true" :show-status-bar="false" />
    <!-- 内容 -->
    <div class="contents">
      <van-tabs v-model:active="active" sticky offset-top="100px">
        <van-tab title="金币收入">
          <div class="wrap">
            <van-pull-refresh v-model="incomeTab.isRefreshing" @refresh="onRefresh('incomeTab')">
              <template v-if="incomeTab.data">
                <van-list v-model:loading="incomeTab.isLoading" finished-text="没有更多了~" :finished="incomeTab.isFinished" :offset="50" :immediate-check="false" @load="onLoad('incomeTab')">
                  <template v-if="incomeTab.data.length > 0">
                    <Item v-for="(item, index) in incomeTab.data" :key="index" :data="item" />
                  </template>
        ...
        <van-tab title="好友贡献">
          <van-pull-refresh v-model="contributionTab.isRefreshing" @refresh="onRefresh('contributionTab')">
            <template v-if="contributionTab.data">
              ...
              >
                <template v-if="contributionTab.data.length > 0">
                  <Item v-for="(item, index) in contributionTab.data" :key="index" :data="item" />
                </template>
                <no-data tips="没有更多啦~" v-else />
              </van-list>
            </template>
        ...
        <van-tab title="金币支出">
          <van-pull-refresh v-model="spendingTab.isRefreshing" @refresh="onRefresh('spendingTab')">
            <template v-if="spendingTab.data">
              <van-list v-model:loading="spendingTab.isLoading" finished-text="没有更多了~" :finished="spendingTab.isFinished" :offset="50" :immediate-check="false" @load="onLoad('spendingTab')">
                <template v-if="spendingTab.data.length > 0">
                  <Item v-for="(item, index) in spendingTab.data" :key="index" :data="item" />
                </template>
                <no-data tips="没有更多啦~" v-else />
              </van-list>
            </template>
