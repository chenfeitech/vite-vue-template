 ...
 * @LastEditors: Lee
 * @LastEditTime: 2022-11-25 21:15:49
 * @Description: 
-->

<template>
  <div class="page px-10">
    <app-header v-if="env !== 'weixin'" title="提现记录" :showBack="true" :show-status-bar="false" />
    <van-pull-refresh v-model="isRefreshing" @refresh="onRefresh">
      <template v-if="data">
        <van-list v-model:loading="isLoading" finished-text="没有更多了~" :finished="isFinished" :offset="50" :immediate-check="false" @load="onLoad">
          <template v-if="data.length > 0">
            <div class="item" v-for="(item, index) in data" :key="index" :data="item">
              <div>
...
interface StateProps extends GD.ListProps<API.RecordItemProps[] | null> {}
...
const getDataSource = async () => {
  const resp = await api.test.records({
    page: state.page,
    pageSize: 20,
  });
  ...
  if (state.type === "refresh" || state.data === null) {
    ...
    state.data = [...state.data!, ...resp.data];
